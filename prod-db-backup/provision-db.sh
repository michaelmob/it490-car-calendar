...
for i in $(ls /vagrant/prod-db-backup/sql/*.sql | sort -g); do
  [ -f "$i" ] || break
  mysql -u root <<< $(sed "1i USE $MYSQL_DB;" "$i")
done

# Install services
cp /vagrant/prod-db-backup/services/log-consumer.service /etc/systemd/system/
cp /vagrant/prod-db-backup/services/auth-consumer.service /etc/systemd/system/
cp /vagrant/prod-db-backup/services/data-consumer.service /etc/systemd/system/
